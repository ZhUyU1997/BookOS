...
KERNEL_SECTORS EQU 128	;now kernel only use 256 sector(256*512/1024 = 128KB)
KERNEL_SECTORS_ALL EQU (128*3)	;now kernel only use 256 sector(256*512/1024 = 128KB)
...
;加载内核
	...
	mov	dword [disk_address_packet +  8], KERNEL_SECTOR_OFFSET
	call	read_sector
	
	mov	dword [disk_address_packet +  6], KERNEL_SEG+0X1000
	mov	dword [disk_address_packet +  8], KERNEL_SECTOR_OFFSET+128
	call	read_sector
	
	mov	dword [disk_address_packet +  6], KERNEL_SEG+0X1000*2
	mov	dword [disk_address_packet +  8], KERNEL_SECTOR_OFFSET+128*2
	call	read_sector
...
disk_address_packet:
	...
	db	KERNEL_SECTORS	; [ 2] Nr of blocks to transfer.
	db	0		; [ 3] Reserved, must be 0.
	dw	0		; [ 4] Addr of transfer - Offset
	dw	KERNEL_SEG	; [ 6] buffer.          - Seg
